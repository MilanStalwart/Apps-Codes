...
import { reactExtension, useApi, Grid, BlockStack, TextBlock, Button, Image, Page, ResourceItem, SkeletonImage, SkeletonText, } from "@shopify/ui-extensions-react/customer-account";
      ...
      <Grid columns={['fill', 'fill', 'fill']} rows="auto" spacing="loose">
        ...
        {!loading && wishlist.length > 0 && wishlist.map((product) => (
          <ResourceItem loading={loading} key={product.id} action={
            <>
              <Button kind='primary' to={"https://milan-dev-2.myshopify.com/products/" + product.handle}>
                View product
              </Button>
              <Button
                kind='secondary'
